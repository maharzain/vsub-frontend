...
const IphoneText = ({ isDark, isTemplate }) => {
  // Extract necessary values from context
  const { username, image, messages } = useContext(fakeTextContext);
  ...
  return (
    <div className='w-full'>
      ...
      {/* Render template message if isTemplate is true */}
      {isTemplate && (
        <section
          className={`chat-right ${
            isDark ? "bg-black" : "bg-white"
          } px-5 pt-[10px] pb-1 flex justify-end`}
        >
          <p className='text-sm text-left text-white bg-iChatRight px-3 py-[7px] max-w-[229px] mb-2 rounded-2xl relative'>
            Dad! I want to marry 😍
            <img src={iTipRight} className='absolute bottom-0 right-[-8px]' />
          </p>
        </section>
      )}

      {/* Render messages only if isTemplate is false */}
      {!isTemplate &&
            ...
            if (msg.type === "Left") {
              ...
              return (
                <section
                  ...
                  className={`chat-left ${
                    isDark ? "bg-black" : "bg-white"
                  } px-5 pt-[10px] pb-1 flex justify-start`}
                >
                  <p
                    className={`text-sm text-left ${
                      isDark
                        ? "text-white bg-[#1A1D29]"
                        : "text-black bg-iChatLeft"
                    } px-3 py-[7px] max-w-[229px] rounded-2xl relative`}
                  >
                    {msg.content.message}
                    <img
          ...
          // Handle image content
            ...
            if (msg.type === "Left") {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[229px] rounded-2xl relative overflow-hidden ${
                      isDark
                        ? "bg-[#1A1D29] text-white"
                        : "bg-iChatLeft text-black"
                    }`}
                  >
                    <img
                      src={msg.content.url}
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[229px] rounded-2xl relative overflow-hidden bg-iChatRight`}
                  >
                    <img
                      src={msg.content.url}
                      alt='Message content'
